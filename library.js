(function(module) {
	...
	*/
	...
	var oauthUserDataRoute = endpointRoot + (process.env.OAUTH_USER_DATA_PATH || '/users/current');
	var constants = Object.freeze({
			type: 'oauth2',	// Either 'oauth' or 'oauth2'
			name: providerName,	// Something unique to your OAuth provider in lowercase, like "github", or "nodebb"
			oauth: {
				requestTokenURL: '',
				accessTokenURL: '',
			...
			userRoute: oauthUserDataRoute // This is the address to your app's "user profile" API endpoint (expects JSON)
		}),
		configOk = false,
	...
	OAuth.deleteUserData = function(data, callback) {
	 ...
	 OAuth.init = function (data, callback) {
        data.router.get('/login', function (req, res) {
            res.redirect('/forum/auth/gameleap');
        });
        data.router.get('/register', function (req, res) {
			var registerRoute = siteRoot + '/signup'
